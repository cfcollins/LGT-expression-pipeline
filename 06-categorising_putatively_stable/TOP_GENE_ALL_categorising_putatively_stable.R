...
## load data
put_stab <- read.csv("expanded_deg_or_stab_TOP_GENE_ALL.csv") %>% 
  as_tibble() %>% 
...
all_categories$Category[49] <- "Intermediate"

write.csv(all_categories, "putatively_stable_categories_top_gene_all.csv", row.names = FALSE)
...
ggsave("putatively_stable_categories_top_gene_all.png")
...
observed_counts <- c(2, 4, 12, 10, 25)
