...
## load data
put_stab <- read.csv("005-degenerating_categorisation_2_truncation/05-combine_with_expression_degenerating/05-singletons/expanded_deg_or_stab_SINGLETONS.csv") %>% 
  as_tibble() %>% 
...
all_categories %>%
  print(n = 51)
all_categories$Category[9] <- "Intermediate"
all_categories$Category[19] <- "Intermediate"

write.csv(all_categories, "006-categorising_putatively_stable/05-singletons/putatively_stable_categories_singletons.csv", row.names = FALSE)
...
ggsave("006-categorising_putatively_stable/05-singletons/putatively_stable_categories_singletons.png")
...
observed_counts <- c(2, 4, 4, 10)
categories <- c("Intermediate", "Donor", "Native", "Same")
...
# Define the observed counts and total counts
observed_counts <- c(2, 4, 4)
categories <- c("Intermediate", "Donor", "Native")
total_counts <- sum(observed_counts)
...
# Perform a binomial test
binom_test <- binom.test(same_count, total_counts, p = 1/3, alternative = "greater")
binom_test
