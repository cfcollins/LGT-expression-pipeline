...
## load data
put_stab <- read.csv("005-degenerating_categorisation_2_truncation/05-combine_with_expression_degenerating/01-sum_all/expanded_deg_or_stab_SUM_ALL.csv") %>% 
  as_tibble() %>% 
...
all_categories$Category[10] <- "Higher"
all_categories$Category[11] <- "Intermediate"
all_categories$Category[13] <- "Higher"
all_categories$Category[16] <- "Higher"
all_categories$Category[20] <- "Higher"
all_categories$Category[21] <- "Intermediate"
all_categories$Category[22] <- "Intermediate"
all_categories$Category[52] <- "Intermediate"

write.csv(all_categories, "putatively_stable_categories_sum_all.csv", row.names = FALSE)
...
summary$Category  <- factor(summary$Category , levels = c("Higher", "Intermediate",
                                                          "Donor", "Native", "Same"))
summary$LGT_native_accession  <- factor(summary$LGT_native_accession , levels = c("AUS1", "ZAM",
                                                          "L04", "KWT", "MRL"))
ggplot(summary, aes(x = LGT_native_accession, y = n, fill = Category)) +
...
ggplot(summary, aes(x = LGT_native_accession, y = pct, fill = Category)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(aes(label = paste0(sprintf("%1.1f", pct * 100), "%", " ", n)), 
            position = position_stack(vjust = 0.5),
            size = 2.5) +
...
ggsave("putatively_stable_categories_sum_all.svg")
