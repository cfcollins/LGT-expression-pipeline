...
## load data
put_stab <- read.csv("expanded_deg_or_stab_SUM_LEAF_SHEATH.csv") %>% 
  as_tibble() %>% 
...
all_categories$Category[9] <- "Higher"
...
all_categories$Category[27] <- "Intermediate"
all_categories$Category[45] <- "Intermediate"

write.csv(all_categories, "putatively_stable_categories_sum_leaf_sheath.csv", row.names = FALSE)
...
ggsave("putatively_stable_categories_sum_leaf_sheath.png")
