...
# accessing data 
counts_w_paralogs <- read.csv("001-Data/002-Counts/LGT_natives_counts.csv") %>% 
  as_tibble() %>%
  filter(Type == "LGT" | Type == "Native") %>% 
  select(LGT.Cluster, Accession, Type, Gene, Sample, logCount)
LGTs_nat <- read.csv("001-Data/001-Lists/LGT & Natives.csv") %>% 
  as_tibble() %>% 
  filter(Type == "LGT" | Type == "Native")
...
singles <- LGTs_nat %>% 
  ...
  summarise(Type = n()) %>% 
  filter(Type == 2)
counts <- semi_join(counts_w_paralogs, singles, by = c("LGT.Cluster", "Accession"))

setwd("003-LGT_vs_native_expression/06-singletons/")
...
write.csv(results, "t-test_results_diff_singletons.csv", row.names = FALSE)
sig <- results %>% 
...
ggsave("LGT_vs_natives_singletons.png", dpi = 1200)

write.csv(counts_combined_columns, "top_gene_singletons.csv", row.names = FALSE)
