...
categories <- read.csv("silenced_lower_vs_not_SINGLETONS.csv") %>% 
  as_tibble() %>% 
...
write.csv(combined, "expanded_deg_or_stab_SINGLETONS.csv", row.names = FALSE)
...
summary <- combined %>% 
  ...
  mutate(pct = n / sum(n),               
         ypos = cumsum(n) - 0.5 * n)
write.csv(summary, "SUMMARY_expanded_deg_or_stab_SINGLETONS.csv")
...
ggsave("expanded_deg_or_stab_SINGLETONS.png")
