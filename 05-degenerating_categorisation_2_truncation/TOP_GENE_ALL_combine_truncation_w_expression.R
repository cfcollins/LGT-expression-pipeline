...
write.csv(combined, "expanded_deg_or_stab_TOP_GENE_ALL.csv", row.names = FALSE)
...
summary <- combined %>% 
  ...
  mutate(pct = n / sum(n),               
         ypos = cumsum(n) - 0.5 * n)
write.csv(summary, "SUMMARY_expanded_deg_or_stab_TOP_GENE_ALL.csv")
...
ggsave("expanded_deg_or_stab_TOP_GENE_ALL.png")
