...
categories <- read.csv("silenced_lower_vs_not_sum_leaf_sheath.csv") %>% 
  as_tibble() %>% 
...
write.csv(combined, "expanded_deg_or_stab_SUM_LEAF_SHEATH.csv", row.names = FALSE)
...
summary <- combined %>% 
  ...
  mutate(pct = n / sum(n),               
         ypos = cumsum(n) - 0.5 * n)
write.csv(summary, "SUMMARY_expanded_deg_or_stab_SUM_LEAF_SHEATH.csv")
...
ggsave("expanded_deg_or_stab_SUM_LEAF_SHEATH.png")
